...
/*
 A Map with mappings of nest identifiers to { () => Item } functions. We use functions so that 'randomElement' will generate
...
private val NESTS = Map(
  ...
  5073 -> (() => SEEDS.randomElement), // Generate random seed
  5074 -> (() => RINGS.randomElement) // Generate random ring
)
