...
private val HERB_TABLE = Map(
  ...
  'ranarr_weed -> Herb(207, 257, 99, 30, 25, 7.5),
...
*/
private abstract class MakePotionAction(plr: Player, level: Int, exp: Double) extends ProducingSkillAction(plr, true, 2) {

  override def canInit = isLevel(plr, level)

  override def onProduce() = {
    plr.sendMessage(onProduceMessage)
    plr.animation(POTION_ANIMATION)

    if (exp > 0.0) {
      plr.skill(HERBLORE).addExperience(exp)
    }
  }
...
private def isLevel(plr: Player, level: Int) = {
  if (plr.skill(HERBLORE).getLevel < level) {
    plr.sendMessage(s"You need a Herblore level of $level to do this.")
    false
  } else {
    true
  }
...
private def cleanHerb(plr: Player, herb: Herb) {
  if (isLevel(plr, herb.cleanLevel)) {
    plr.interruptAction()
