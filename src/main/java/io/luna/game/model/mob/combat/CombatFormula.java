...
public final class CombatFormula {
    ...
    public static int computeDefenceRoll(Mob attacker, Mob victim, CombatDamageType type) {
        ...
        return (int) (Math.floor(level * getDefencePrayerMultiplier(player))
                + getDefenceStanceBonus(player, type) + 8)
                * (getDefenceBonus(attacker, player) + 64);
    }
     ...
     */
    public static int getDefenceBonus(Mob attacker, Player victim) {
        EquipmentBonus bonus = attacker.getCombat().getWeapon().getStyleDef().getBonus();
        return victim.getEquipment().getBonus(bonus.getOpposite());
    }
