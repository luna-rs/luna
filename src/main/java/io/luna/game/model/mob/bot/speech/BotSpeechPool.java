...
public class BotSpeechPool<T extends Enum<T>> {
     ...
     * The dynamic tags.
     */
    private final Table<T, String, Function<Bot, Object>> tags = HashBasedTable.create();
     ...
     * <pre><code>
     *     speechPool.setTag(SpeechContext.GREETINGS, "name", bot -> bot.getUsername());
     *     speechPool.setTag(SpeechContext.TRAINING, "skill", bot -> Skill.name(skillId));
     *     speechPool.setTag(SpeechContext.TRAINING, "level", bot -> bot.getSkill(skillId).getLevel());
     * </code></pre>
     *
     ...
     *
     * @param speechType The speech context this tag will be assigned to.
     ...
     */
    public void setTag(T speechType, String name, Function<Bot, Object> value) {
        tags.put(speechType, name, value);
    }
    ...
    public final String take(Bot bot, T context) {
        ...
        if (phrase != null) {
            for (var entry : tags.row(context).entrySet()) {
