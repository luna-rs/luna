...
on(CombatDamageReceivedEvent::class) {
    if (plr.equipment.ring?.id == 2570 && plr.healthPercent <= DegradableEquipmentHandler.RING_OF_LIFE_HEALTH_PERCENT &&
        !plr.combat.magic.isTeleBlocked && plr.isAlive) {
        ...
        damage.attacker.combat.target = null // Reset attacker's target.
    }
}