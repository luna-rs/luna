 ...
 */
val Player.jingleThrottle by Attr.timer()
...
fun advanceLevel(plr: Player, skillId: Int, oldLevel: Int) {
    ...
    if (oldLevel < newLevel) {
        ...
        if(plr.jingleThrottle.durationTicks >= 8) {
            val jingle = if (MILESTONE_LEVELS.contains(newLevel)) MILESTONE_JINGLES[skillId]
            else LEVEL_UP_JINGLES[skillId]
            if (jingle != null) {
                plr.queue(MusicMessageWriter(jingle.id))
                plr.jingleThrottle.reset()
            }
        }
