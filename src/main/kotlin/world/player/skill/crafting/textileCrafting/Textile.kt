...
enum class Textile(val rawItem: Item, val processedItem: Item, val level: Int, val exp: Double, val type: TextileType) {
    ...
    companion object {
         ...
         */
        val SPINNING_WHEEL_PROCESSED_IDS =
            values().filter { it.type == TextileType.SPINNING_WHEEL }.map { it.processedItem.id }.toIntArray()

        /**
         * An array of loom based processed items.
         */
        val LOOM_PROCESSED_IDS =
            values().filter { it.type == TextileType.LOOM }.map { it.processedItem.id }.toIntArray()
    }
}